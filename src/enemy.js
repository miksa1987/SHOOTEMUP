...
const draw = (enemy, ctx) => {
  ...
  ctx.lineWidth = 1
  ctx.beginPath()
  ctx.arc(enemy.x, enemy.y, enemy.radius / 2 * RADIUS_MULTIPLIER, 0, 2 * Math.PI)
  ctx.stroke()
  ctx.fill()
}
...
const move = (enemy) => {
  ...
  if(enemy.y < -(enemy.radius * RADIUS_MULTIPLIER)) enemy.y = CANVAS_HEIGHT + (enemy.radius * RADIUS_MULTIPLIER)
  if(enemy.x > CANVAS_WIDTH + (enemy.radius * RADIUS_MULTIPLIER)) enemy.x = -(enemy.radius * RADIUS_MULTIPLIER)
  if(enemy.y > CANVAS_HEIGHT + (enemy.radius * RADIUS_MULTIPLIER)) enemy.y = -(enemy.radius * RADIUS_MULTIPLIER)
}
